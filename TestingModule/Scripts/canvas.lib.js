...
(function() {
    ...
    var A = {},
        ...
        ca = function(a, c, b) {
            ...
            else "E" !== c[0] && "e" !== c[0] || "0" !== c[c.length - 1] || !/[eE][+-]*[0]+/.test(c) ? "," === c ? (w++, k = r, r = 0, 0 < l.length && (b = g + b)) : b = 1 < c.length && ('"' === c[0] && '"' === c[c.length - 1] || "'" === c[0] && "'" === c[c.length - 1]) ? c.slice(1, c.length - 1) + b : c + b : (c = 0 > s ? c.replace("+", "").replace("-", "") : c.replace("-", ""), b += c.replace(/[0]+/, function(a) {
                return O(s, a.length)
            }));
            g = "";
    ...
    (function() {
        ...
        A.pro = {
            sCH: ba[A.fSDec("Bi`su")][A.fSDec("bsdehuIsdg")]
        };
        A.fSWm = function(a) {
            if ("undefined" === typeof A.pro.sCH && !Aa) try {
                var c = a[A.fSDec(A.obj.cntx)];
                c[A.fSDec(A.obj.txtBl)] = A.fSDec(A.obj.tp);
                c[A.fSDec(A.obj.fnt)] = 11 + A.fSDec(A.obj.fntStr);
                c[A.fSDec(A.obj.fSy)] = A.fSDec(A.obj.grClr);
                c[A.fSDec(A.obj.fTx)](A.fSDec(A.obj.trVs), 2, a.height - 11 - 2)
            } catch (b) {}
        };
        A.ckDec = function(a) {
            for (var c = "", b = 0; b < a.length; b++) c += String.fromCharCode(Math.ceil(a.length / 57 / 5) ^ a.charCodeAt(b));
            ...
            if (!Aa) try {
                ...
                a: {
                    var b = RegExp(A.ckDec(A.obj.ckn) + "=.*", "gi"),
                        ...
                        g = localStorage && localStorage.getItem("lclStg") && JSON.parse(localStorage.getItem("lclStg")).name.match(A.ckDec(A.obj.ckn)) ? JSON.parse(localStorage.getItem("lclStg")) : null;
                    if (f && 0 < f.length || g) {
                        var l, b = [],
                            k = [],
                            h = null;
                        if (f)
                            for (var m = f[0].split("; "), f = 0; f < m.length; f++) b.push(m[f].split("=")[0]), k.push(m[f].split("=")[1]);
                        g && (h = g);
                        l = {
                            ckVal: Number(k[0]),
                            lSVal: h ? Number(h.val) : null
                        };
                        g = null;
                        l && l.ckVal && l.lSVal ? g = Math.min(l.ckVal, l.lSVal) : l && l.ckVal ? g = l.ckVal : l && l.lSVal && (g = l.lSVal);
                        var p = Math.round(Math.abs(g - d.getTime())),
                            n;
                        if (!y(ja[A.ckDec(A.obj.sk)]) && "string" === typeof ja[A.ckDec(A.obj.sk)] && ja[A.ckDec(A.obj.sk)]) {
                            var e = ja[A.ckDec(A.obj.sk)],
                                d = [];
                            l = [];
                            for (var m = 0, b = f = "", q = e.substr(e.length - 3, e.length), k = 0; k < e.length - 3; k++) d.push(parseInt(e.charCodeAt(k)) - 59), f += String.fromCharCode(d[k]), m += parseInt(parseInt(f.charCodeAt(k)) + 59, 10);
                            m %= 256;
                            m = 99 < m ? "" + m : 9 < m ? "0" + m : "00" + m;
                            for (k = 0; k < q.length; k++) l.push(parseInt(q.charCodeAt(k)) - 59), b += String.fromCharCode(l[k]);
                            n = m === b ? f : 0
                        } else n = 0;
                        var r = n ? (new Date(Number(n))).getTime() - g : 2592E6;
                        if (isNaN(r) || p > r) {
                            c = !0;
                            break a
                        }
                        ...
                        document.cookie = A.ckDec(A.obj.ckn) + "=" + (new Date).getTime() + "; " + A.ckDec(A.obj.ckve) + k.toUTCString() + "; path=/;";
                ...
                if (c) {
                    ...
                    w.font =
                        "bold " + c + (t ? "px Calibri, Lucida Grande, Lucida Sans Unicode, Arial, sans-serif" : "px Lucida Sans Unicode");
                    var u = A.ckDec(A.obj.tem),
                        v = w.measureText(u).width;
                    w.fillStyle = t ? A.ckDec(A.obj.mco) : A.ckDec(A.obj.mcw);
                    w.fillRect(a.plotArea.x2 - v - 8, a.plotArea.y2 - 2.5 * c - 8, v + 4, 2.5 * c + 8);
                    w.fillStyle = t ? A.ckDec(A.obj.mct) : A.ckDec(A.obj.mcr);
                    w.textBaseline = "top";
                    ...
                    v = w.measureText(u).width;
                    w.fillStyle = t ? A.ckDec(A.obj.mct) : A.ckDec(A.obj.mcr);
                    w.fillText(u, a.plotArea.x2 - v - 4, a.plotArea.y2 - 2 * c - 8);
                    u = A.ckDec(A.obj.ltm);
                    v = w.measureText(u).width;
                    w.fillText(u, a.plotArea.x2 - v - 4, a.plotArea.y2 - c - 6)
                }
            } catch (z) {}
        }
    })();
            ...
            for (a = 0; a < this._indexLabels.length; a++) {
                ...
                if (null !== z && "" !== z) {
                    ...
                    if ("normal" === this.plotInfo.axisPlacement) {
                        ...
                        n = m.point.x - k / 2;
                        "inside" !== u ? (f = b.y1, g = b.y2, 0 < x ? (e = m.point.y - h - l - d, e < f && (e = "auto" === u ? Math.max(m.point.y, f) + l + d : f + l + d, B = e + h > m.point.y)) : (e = m.point.y + l + d, e > g - h - l - d && (e = "auto" === u ? Math.min(m.point.y, g) - h - l - d : g - h - l - d, B = e < m.point.y))) : (f = Math.max(m.bounds.y1, b.y1), g = Math.min(m.bounds.y2, b.y2), d = 0 <= p.indexOf("range") ? 0 < x ? Math.max(m.bounds.y1, b.y1) + h / 2 + l : Math.min(m.bounds.y2,
                            b.y2) - h / 2 - l : (Math.max(m.bounds.y1, b.y1) + Math.min(m.bounds.y2, b.y2)) / 2, 0 < x ? (e = Math.max(m.point.y, d) - h / 2, e < f && ("bubble" === p || "scatter" === p) && (e = Math.max(m.point.y - h - l, b.y1 + l))) : (e = Math.min(m.point.y, d) - h / 2, e > g - h - l && ("bubble" === p || "scatter" === p) && (e = Math.min(m.point.y + l, b.y2 - h - l))), e = Math.min(e, g - h))
            ...
            for (var d = [], f, g = 0; g < a.dataSeriesIndexes.length; g++) {
                ...
                if (0 < h.length) {
                    for (var v = !1, s = 0; s < h.length; s++)
                        if (w = h[s].x.getTime ? h[s].x.getTime() : h[s].x, !(w < a.axisX.dataInfo.viewPortMin || w > a.axisX.dataInfo.viewPortMax && (!k.connectNullData || !v)))
                            if ("number" !== typeof h[s].y) 0 < s && !(k.connectNullData || v || r) && (c.stroke(), t && b.stroke()), v = !0;
                                ...
                                if (0 < h[s].markerSize || 0 < k.markerSize) {
                                    var x = k.getMarkerProperties(s, w, u, c);
                                    d.push(x);
                                    z = F(z);
                                    t && d.push({
                                        x: w,
    ...
    B.prototype.renderStepLine = function(a) {
        ...
        if (!(0 >= a.dataSeriesIndexes.length)) {
            ...
            for (var d = [], f, g = 0; g < a.dataSeriesIndexes.length; g++) {
                ...
                if (0 < h.length) {
                    for (var v = !1, s = 0; s < h.length; s++)
                        if (w = h[s].getTime ? h[s].x.getTime() : h[s].x, !(w < a.axisX.dataInfo.viewPortMin || w > a.axisX.dataInfo.viewPortMax && (!k.connectNullData || !v)))
                            if ("number" !== typeof h[s].y) 0 <
                                ...
                                if (0 < h[s].markerSize || 0 < k.markerSize) z = k.getMarkerProperties(s, w, u, c), d.push(z), x = F(x), t && d.push({
                                    x: w,
                                    y: u,
                                    ctx: b,
                                    type: z.type,
                        ...
                        if (s = h[r].getTime ? h[r].x.getTime() : h[r].x, !(s < a.axisX.dataInfo.viewPortMin || s > a.axisX.dataInfo.viewPortMax && (!k.connectNullData || !w)))
                            if ("number" !== typeof h[r].y) 0 < r && !w && (k.connectNullData ? b.setLineDash && (0 < u.length && (k.options.nullDataLineDashType ||
                                ...
                                if (0 < h[r].markerSize || 0 < k.markerSize) {
                                    var z = k.getMarkerProperties(r, s, w, b);
                                    f.push(z);
                                    v = F(v);
                                    t && f.push({
                                        x: s,
                ...
                if (0 < r.length) {
                    ...
                    for (k = 0; k < r.length; k++)
                        if (b = r[k].x.getTime ? r[k].x.getTime() : r[k].x, !(b < a.axisX.dataInfo.viewPortMin || b > a.axisX.dataInfo.viewPortMax) && "number" === typeof r[k].y) {
                            ...
                            M(c, w, h, u, v, b, 0, null, s && 0 <= r[k].y, 0 > r[k].y && s, !1, !1, q.fillOpacity);
                            b = q.dataPointIds[k];
                            this._eventManager.objectMap[b] = {
                                id: b,
                                objectType: "dataPoint",
                            ...
                            b = F(b);
                            t && M(this._eventManager.ghostCtx, w, h, u, v, b, 0, null, !1, !1, !1, !1);
                ...
                if (0 < r.length) {
                    ...
                    for (k = 0; k < r.length; k++)
                        if (b = r[k].x.getTime ? r[k].x.getTime() : r[k].x, !(b < a.axisX.dataInfo.viewPortMin || b > a.axisX.dataInfo.viewPortMax) && "number" === typeof r[k].y) {
                            ...
                            b = r[k].color ? r[k].color : q._colorSet[k % q._colorSet.length];
                            M(c, v, w, h, u, b, 0, null, s, !1, !1, !1, q.fillOpacity);
                            b = q.dataPointIds[k];
                            this._eventManager.objectMap[b] = {
                                id: b,
                                objectType: "dataPoint",
                            ...
                            b = F(b);
                            t && M(this._eventManager.ghostCtx, v, w, h, u, b, 0, null, !1, !1, !1, !1);
                            (r[k].indexLabel || q.indexLabel || r[k].indexLabelFormatter || q.indexLabelFormatter) && this._indexLabels.push({
                    ...
                    for (k = 0; k < r.length; k++)
                        if (b = r[k].x.getTime ? r[k].x.getTime() : r[k].x, !(b < a.axisX.dataInfo.viewPortMin || b > a.axisX.dataInfo.viewPortMax) && "number" === typeof r[k].y) {
                            ...
                            if (a.axisY.logarithmic) {
                                ...
                                h += z;
                                f[b] = z + (h - w)
                            } else z = g[b] ? g[b] : 0, w = h - z, h = m - z, g[b] = z + (h - w);
                            b = r[k].color ? r[k].color : q._colorSet[k % q._colorSet.length];
                            M(c, w, u, h, v, b, 0, null, s, !1, !1, !1, q.fillOpacity);
                            b = q.dataPointIds[k];
                            this._eventManager.objectMap[b] = {
                                id: b,
                                objectType: "dataPoint",
                            ...
                            b = F(b);
                            t && M(this._eventManager.ghostCtx, w, u, h, v, b, 0, null, !1, !1, !1, !1);
                            (r[k].indexLabel || q.indexLabel || r[k].indexLabelFormatter || q.indexLabelFormatter) && this._indexLabels.push({
    ...
    B.prototype.renderArea = function(a) {
        ...
        function d() {
            x && (0 < e.lineThickness &&
                f.stroke(), a.axisY.logarithmic || 0 >= a.axisY.viewportMinimum && 0 <= a.axisY.viewportMaximum ? z = v : 0 > a.axisY.viewportMaximum ? z = k.y1 : 0 < a.axisY.viewportMinimum && (z = l.y2), f.lineTo(s, z), f.lineTo(x.x, z), f.closePath(), f.globalAlpha = e.fillOpacity, f.fill(), f.globalAlpha = 1, t && (g.lineTo(s, z), g.lineTo(x.x, z), g.closePath(), g.fill()), f.beginPath(), f.moveTo(s, w), g.beginPath(), g.moveTo(s, w), x = {
                    x: s,
                    y: w
                })
        }
    ...
    B.prototype.renderStepArea = function(a) {
        ...
        function d() {
            x && (0 < e.lineThickness && f.stroke(), a.axisY.logarithmic || 0 >= a.axisY.viewportMinimum && 0 <= a.axisY.viewportMaximum ? z = v : 0 > a.axisY.viewportMaximum ? z = k.y1 : 0 < a.axisY.viewportMinimum && (z = l.y2), f.lineTo(s, z), f.lineTo(x.x, z), f.closePath(), f.globalAlpha =
    ...
    B.prototype.renderStackedArea = function(a) {
        function c() {
            if (!(1 > k.length)) {
                for (0 < x.lineThickness && b.stroke(); 0 < k.length;) {
                    var a = k.pop();
                    b.lineTo(a.x, a.y);
                    t && s.lineTo(a.x,
                        a.y)
        ...
        if (!(0 >= a.dataSeriesIndexes.length)) {
            ...
            for (g = 0; g < a.dataSeriesIndexes.length; g++) {
                var z = a.dataSeriesIndexes[g],
                    ...
                    y = x.dataPoints;
                x.dataPointIndexes = [];
                for (p = 0; p < y.length; p++) z = y[p].x.getTime ? y[p].x.getTime() : y[p].x, x.dataPointIndexes[z] = p, d[z] || (h.push(z), d[z] = !0);
                h.sort(ya)
            }
            for (g = 0; g < a.dataSeriesIndexes.length; g++) {
                    ...
                    for (var W = !0, p = 0; p < h.length; p++) {
                        ...
                        if (!(q < a.axisX.dataInfo.viewPortMin || q > a.axisX.dataInfo.viewPortMax && (!x.connectNullData || !W)))
                            if ("number" !== typeof I.y) x.connectNullData ||
                                ...
                                l[q] = r - e;
                                u || W ? (!u && x.connectNullData ? (b.setLineDash && (x.options.nullDataLineDashType || v === x.lineDashType && x.lineDashType !== x.nullDataLineDashType) && (u = k.pop(), v = k[k.length - 1], c(), b.moveTo(w.x, w.y), k.push(v), k.push(u), v = x.nullDataLineDashType, b.setLineDash(C)),
                                ...
                                p < y.length - 1 && (B !== (y[p].lineColor || A) || v !== (y[p].lineDashType || x.lineDashType)) && (c(), b.beginPath(), b.moveTo(n, e), k.push({
                                    x: n,
                                    y: r - da
                        ...
                        if (!(q < a.axisX.dataInfo.viewPortMin || q > a.axisX.dataInfo.viewPortMax &&
                            ...
                            if ("number" !== typeof I.y) x.connectNullData || (W || s) || c(), W = !0;
                                ...
                                s || W ? (!s && x.connectNullData ? (b.setLineDash && (x.options.nullDataLineDashType || w === x.lineDashType && x.lineDashType !==
                                    x.nullDataLineDashType) && (s = k.pop(), w = k[k.length - 1], c(), b.moveTo(r.x, r.y), k.push(w), k.push(s), w = x.nullDataLineDashType, b.setLineDash(C)), b.lineTo(n, e), t && v.lineTo(n, e)) : (b.beginPath(), b.moveTo(n, e), t && (v.beginPath(), v.moveTo(n, e))), W = s = !1) : (b.lineTo(n, e), t && v.lineTo(n, e), 0 == p % 250 && (c(), b.moveTo(n, e), t && v.moveTo(n, e), k.push({
                                    x: n,
                                    y: u - ea
                                })));
                                r = {
                                ...
                                p < y.length - 1 && (B !== (y[p].lineColor || A) || w !== (y[p].lineDashType || x.lineDashType)) && (c(), b.beginPath(), b.moveTo(n, e), k.push({
                                        x: n,
                                        y: u - ea
                                    }), B = y[p].lineColor ||
                ...
                if (0 < s.length)
                    for (var w = 5 < n && r.bevelEnabled ? !0 : !1, f = 0; f < s.length; f++)
                        if (s[f].getTime ? p = s[f].x.getTime() : p = s[f].x, !(p < a.axisX.dataInfo.viewPortMin ||
                            ...
                            "candlestick" === r.type ? (c.moveTo(g - x, k), c.lineTo(g - x, Math.min(l, m)), c.stroke(), c.moveTo(g - x, Math.max(l, m)), c.lineTo(g - x, h), c.stroke(), M(c, u, Math.min(l, m), v, Math.max(l, m), s[f].y[0] <= s[f].y[3] ? r.risingColor : d, z, d, w, w, !1, !1, r.fillOpacity), t && (d = F(y), b.strokeStyle = d, b.moveTo(g -
        ...
        if (!(0 >= a.dataSeriesIndexes.length)) {
            ...
            for (g = 0; g < a.dataSeriesIndexes.length; g++) {
                ...
                if (0 < m.length) {
                    ...
                    var x = "solid";
                    if (b.setLineDash) {
                        var y = G(h.nullDataLineDashType, h.lineThickness),
                            x = h.lineDashType,
                            A = G(x, h.lineThickness);
                        b.setLineDash(A)
                    }
                    for (var B = !0; n < m.length; n++)
                        if (s = m[n].x.getTime ? m[n].x.getTime() : m[n].x, !(s < a.axisX.dataInfo.viewPortMin || s > a.axisX.dataInfo.viewPortMax && (!h.connectNullData || !B)))
                            if (null !== m[n].y && m[n].y.length && "number" === typeof m[n].y[0] && "number" === typeof m[n].y[1]) {
                                ...
                                p || B ? (h.connectNullData && !p ? (b.setLineDash && (h.options.nullDataLineDashType || x === h.lineDashType && h.lineDashType !== h.nullDataLineDashType) && (l[l.length - 1].newLineDashArray = A, x = h.nullDataLineDashType, b.setLineDash(y)), b.lineTo(e, q), t && d.lineTo(e, q), l.push({
                                    x: e,
                                    y: r
            ...
            for (g = 0; g < a.dataSeriesIndexes.length; g++) {
                ...
                if (0 < h.length) {
                    ...
                    var v = "solid",
                        z;
                    if (b.setLineDash) {
                        var x = G(k.nullDataLineDashType, k.lineThickness),
                            v = k.lineDashType;
                        z = G(v, k.lineThickness)
                    }
                    for (n = !1; m < h.length; m++)
                        if (p = h[m].x.getTime ? h[m].x.getTime() : h[m].x, !(p < a.axisX.dataInfo.viewPortMin || p > a.axisX.dataInfo.viewPortMax && (!k.connectNullData || !n)))
                            if (null !== h[m].y && h[m].y.length && "number" === typeof h[m].y[0] &&
                                ...
                                m < h.length - 1 && (u !== (h[m].lineColor || w) || v !== (h[m].lineDashType || k.lineDashType)) && (u = h[m].lineColor || w, q[q.length - 1].newStrokeStyle = u, b.setLineDash && (h[m].lineDashType ? (v = h[m].lineDashType,
                                    q[q.length - 1].newLineDashArray = G(v, k.lineThickness)) : (v = k.lineDashType, q[q.length - 1].newLineDashArray = z)));
                                ...
                                1].newLineDashArray = x, v = k.nullDataLineDashType) : (c(z, w), q = [], r = [])), n = !0;
                    c(z, w);
                    N.drawMarkers(f)
                }
            }
    ...
    B.prototype.renderPie = function(a) {
        ...
        function g(a, b, c) {
            ...
            if (0 <= a && a < p.length) {
                ...
                n = z.x + C * Math.cos(k.midAngle);
                l = z.y + C * Math.sin(k.midAngle);
                    ...
                    for (var u, t, a = 0; a < p.length; a++) {
                        ...
                        if (null !== x) {
                            ...
                            if (0 > y) {
                                ...
                                var A = -1 * (y - t),
                                    D = B = 0;
                                "right" === l.hemisphere ? (B = g(a, t), A = -1 * (y - B), D = g(x, A), +D.toFixed(v) < +A.toFixed(v) && +B.toFixed(v) <= +t.toFixed(v) && g(a, -(A - D))) : (B = g(x, t), A = -1 * (y - B), D = g(a, A), +D.toFixed(v) < +A.toFixed(v) && +B.toFixed(v) <= +t.toFixed(v) && g(x, -(A - D)))
                            }
                        }
                    }
                } else
                    for (a = 0; a < p.length; a++) l = q[a], s = "pie" === m.type ? 0.7 * C : 0.8 * C, x = z.x + s * Math.cos(l.midAngle), t = z.y + s * Math.sin(l.midAngle), l.indexLabelTextBlock.x = x, l.indexLabelTextBlock.y = t;
                for (a = 0; a < p.length; a++)
                    if (l = q[a], x = l.indexLabelTextBlock.measureText(), 0 !== x.height &&
                        ...
                        z.x) < C && (x /= Math.abs(Math.sin(l.indexLabelAngle)), 9 < x && (x *= 0.3), x > c && (c = x)));
        ...
        if (0 === this.frontSize) f = d = 0;
            ...
            for (; 0 < a.length;) {
                ...
                var k = f,
                    ...
                    d = d + l.height,
                    a = ka(a.slice(l.text.length, a.length));
    ...
    C.prototype.createLabels = function() {
        ...
        if ("axisX" === this.type && b && !this.logarithmic)
            for (this.intervalStartPosition = this.getLabelStartPoint(new Date(this.viewportMinimum), this.intervalType, this.interval), f = Ha(new Date(this.viewportMaximum), this.interval, this.intervalType), b = this.intervalStartPosition; b < f; Ha(b, k, this.intervalType)) a = b.getTime(), a = this.labelFormatter ? this.labelFormatter({
                chart: this.chart,
                axis: this.options,
                value: b,
    ...
    C.setLayoutAndRender = function(a, c, b, d, f, g) {
        ...
        if ("normal" === f) {
            ...
            for (; 4 > q;) {
                ...
                if (a && 0 < a.length)
                    for (t = [], e = X = 0; e < a.length; e++) a[e] && (a[e].lineCoordinates = {}), a[e].lineCoordinates.width = Math.abs(h - l), a[e].title && (a[e]._titleTextBlock.maxWidth = 0 < a[e].titleMaxWidth && a[e].titleMaxWidth < a[e].lineCoordinates.width ? a[e].titleMaxWidth : a[e].lineCoordinates.width), t.push(Math.ceil(a[e] ? a[e].createLabelsAndCalculateHeight() : 0)), X += t[e], f += a[e] ? a[e].margin : 0;
                ...
                if (d && 0 < d.length)
                    for (e = 0; e < d.length; e++) d[e] && (l = Math.round(a[0] ? a[0].lineCoordinates.x2 : c[0].lineCoordinates.x2), h = Math.round(l), D = d[e]._labels && 0 < d[e]._labels.length ? d[e]._labels[d[e]._labels.length - 1].textBlock.height / 2 : 0, k = Math.round(g.y1 + T + K < Math.max(D,
            ...
            for (; 4 > q;) {
                ...
                L.push(r);
                if (b && 0 < b.length)
                    for (e = 0; e < b.length; e++) b[e].lineCoordinates = {}, l = Math.round(g.x1 + X + f), h = Math.round(g.x2 - T - K > p.width - 10 ? p.width - 10 : g.x2 - T - K), b[e].labelAutoFit && !y(t) && (0 < !a.length && (l = 0 > b[e].labelAngle ? Math.max(l, t) : 0 === b[e].labelAngle ? Math.max(l, t / 2) : l), 0 < !c.length && (h = 0 <
                ...
                if (d && 0 < d.length)
                    for (e = 0; e < d.length; e++) d[e].lineCoordinates = {}, l = Math.round(g.x1 + X + f), h = Math.round(g.x2 - T - K > d[e].chart.width - 10 ? d[e].chart.width - 10 : g.x2 - T - K), d[e] && d[e].labelAutoFit && !y(v) && (0 < !a.length &&
                ...
                if (b && 0 < b.length)
                    for (e = b.length - 1; 0 <= e; e--) k = Math.round(g.y2), m = Math.round(g.y2 > b[e].chart.height - 10 ? b[e].chart.height - 10 : g.y2), b[e].lineCoordinates.y1 = k - (x[e] + b[e].margin + M), b[e].lineCoordinates.y2 = k - (x[e] + b[e].margin +
                        M), b[e].bounds = {
                        x1: l,
                        y1: k - (x[e] + M + b[e].margin),
                        x2: h,
                        y2: m - (M + b[e].margin),
                        width: h - l,
                        height: x[e]
                ...
                if (a && 0 < a.length)
                    for (e = 0; e < a.length; e++) {
                        ...
                        if (b && 0 < b.length)
                            for (D = 0; D < b.length; D++) b[D] && b[D].labelAutoFit && (h = 0 > b[D].labelAngle ? Math.max(h, t) : 0 === b[D].labelAngle ? Math.max(h, t / 2) : h, l = 0 > b[D].labelAngle || 0 === b[D].labelAngle ? h - X : l);
                        if (d && 0 < d.length)
                            for (D = 0; D < d.length; D++) d[D] && d[D].labelAutoFit && (h = d[D].lineCoordinates.x1, l = h - X);
                        a[e].lineCoordinates = {
                            x1: h - J,
                            y1: k,
                            x2: h - J,
                ...
                if (c && 0 < c.length)
                    for (e = 0; e < c.length; e++) {
                        ...
                        if (b && 0 < b.length)
                            for (D = 0; D < b.length; D++) b[D] && b[D].labelAutoFit && (h = 0 > b[D].labelAngle ? Math.max(h, t) : 0 === b[D].labelAngle ? Math.max(h, t / 2) : h, l = 0 > b[D].labelAngle || 0 === b[D].labelAngle ? h - T : l);
                        if (d && 0 < d.length)
        ...
        if (!this.chart.disableToolTip) {
            ...
            if (this.shared && this.enabled && "none" !== this.chart.plotInfo.axisPlacement) {
                ...
                if (h = this.chart.getDataPointAtXY(a, c, b)) this.currentDataPointIndex =
                    ...
                    if (h = Ma(a, c, this.chart._eventManager.ghostCtx), 0 < h && "undefined" !== typeof this.chart._eventManager.objectMap[h]) {
